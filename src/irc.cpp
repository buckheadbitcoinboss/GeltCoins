...
// Copyright (c) 2011-2012 GeltCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GeltCoinTEST0\r");
            Send(hSocket, "WHO #GeltCoinTEST0\r");
        } else {
            // randomly join #GeltCoin00-#GeltCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #GeltCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GeltCoin%02d\r", channel_number).c_str());
        }
