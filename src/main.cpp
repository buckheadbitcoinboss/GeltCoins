...
uint256 hashGenesisBlock("0x5969626dcf71756faa145c0b21593d7e8ba36aafcd6008c7b12f11914651c86f");
...
const string strMessageMagic = "GeltCoin Signed Message:\n";
...
{
    // GeltCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // GeltCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "gelt genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x614a9b755fcea98f7d840ca97f5523c9427c44f52be364ca865048abb91992ed"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xdd, 0xc4, 0xa5, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GeltCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GeltCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GeltCoinMiner(CWallet *pwallet)
{
    printf("GeltCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GeltCoin-miner");
    ...
    try { loop {
        ...
        printf("Running GeltCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GeltCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GeltCoinMiner, pwallet));
}
